...
class FFmpegManager:
    def __init__(self, sample_rate: int = 16000, channels: int = 1):
        ...
        self.on_error_callback: Optional[Callable[[str], Awaitable[None]]] = None
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(f"FFmpeg process created with PID: {self.process.pid}")
            
            # Start stderr monitoring immediately
            self._stderr_task = asyncio.create_task(self._drain_stderr())
            
            # Add a small delay to let FFmpeg initialize
            await asyncio.sleep(0.1)
            
            # DIAGNOSTIC: Check if process is still alive
            if self.process.returncode is not None:
                logger.error(f"FFmpeg process terminated immediately with return code: {self.process.returncode}")
                async with self._state_lock:
                    self.state = FFmpegState.FAILED
                return False
            
            # DIAGNOSTIC: Verify stdin and stdout are available
            if not self.process.stdin:
                logger.error("FFmpeg stdin is not available")
                async with self._state_lock:
                    self.state = FFmpegState.FAILED
                return False
            ...
            # DIAGNOSTIC: Check if stdin is writable
            if self.process.stdin.is_closing():
                logger.error("FFmpeg stdin is already closing")
                async with self._state_lock:
                    self.state = FFmpegState.FAILED
                return False
            ...
            logger.info(f"FFmpeg started successfully. PID: {self.process.pid}, State: {self.state}")
            return True
    ...
    async def write_data(self, data: bytes) -> bool:
        ...
        try:
            if self.process and self.process.stdin:
                # DIAGNOSTIC: Check process health before writing
                if self.process.returncode is not None:
                    logger.error(f"FFmpeg process has terminated with return code: {self.process.returncode}")
                    async with self._state_lock:
                        self.state = FFmpegState.FAILED
                    return False
                
                # DIAGNOSTIC: Check if stdin is still open
                if self.process.stdin.is_closing():
                    logger.error("FFmpeg stdin is closing/closed")
                    async with self._state_lock:
                        self.state = FFmpegState.FAILED
                    return False
                ...
                logger.debug(f"Successfully wrote {len(data)} bytes to FFmpeg")
                return True
            else:
                logger.error("FFmpeg process or stdin is None")
                return False
        except Exception as e:
            logger.error(f"Error writing to FFmpeg: {e}")
            # DIAGNOSTIC: Check if process died during write
            if self.process and self.process.returncode is not None:
                logger.error(f"FFmpeg process died during write, return code: {self.process.returncode}")
            if self.on_error_callback:
    ...
    async def _drain_stderr(self):
        try:
            while self.process and self.process.stderr:
                ...
                if not line:
                    break
                stderr_msg = line.decode(errors='ignore').strip()
                if stderr_msg:
                    # DIAGNOSTIC: Log stderr output at error level for better visibility
